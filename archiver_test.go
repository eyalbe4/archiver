...
func TestRestoreDirMode(t *testing.T) {
	...
	restoredMode := fileInfo.Mode().Perm()
	if restoredMode != originalMode.Perm() {
		t.Errorf("Directory mode not restored. Expected: %s, Actual: %s", originalMode, restoredMode)
	}
}
